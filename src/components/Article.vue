      ...
      <div v-else>
        <div class="topic_header">
          ...
          <ul>
            ...
            <li>• 作者 {{post.author ? post.author.loginname : ''}}</li>
        ...
        <div class="reply">
          <div class="topbar">{{post.replies ? post.replies.length : 0}} 回复：</div>
          <div v-for="(reply, index) in post.replies" :key="index" class="replySec">
            <div class="replyUp">
              <router-link :to="{
                name:'userinfo',
                params:{
                  name: reply.author ? post.author.loginname : ''
                }
              }">
                <img class="avatar" :src="reply.author ? reply.author.avatar_url : ''">
              </router-link>
              <router-link :to="{
                name:'userinfo',
                params:{
                  name: reply.author? reply.author.loginname : ''
                }
              }">
                <span>{{reply.author? reply.author.loginname : ''}}</span>
              </router-link>
              <span>#{{index+1}}楼</span>
              <span v-if="reply.ups && reply.ups.length > 0">
              ☝ {{reply.ups.length}}
              </span>
              <span v-else=""></span>
